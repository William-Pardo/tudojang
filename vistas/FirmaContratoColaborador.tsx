...
// Espejo de FirmaContrato.tsx (flujo de firma de estudiantes), adaptado para el
// Contrato Laboral/Prestación de Servicios del Equipo Técnico: el colaborador firma
// para sí mismo, sin concepto de tutor.
import React from 'react';
import * as ReactRouterDOM from 'react-router-dom';
import { usePaginaFirmaContratoColaborador } from '../hooks/usePaginaFirmaContratoColaborador';
...
const VistaFirmaContratoColaborador: React.FC = () => {
    ...
    const {
        ...
        limpiarFirma,
        enviarFirma,
    } = usePaginaFirmaContratoColaborador({ idUsuario });

    const renderContent = () => {
        if (cargando) return <Loader texto="Cargando Contrato Laboral..." />;
        if (error) return (
        ...
        if (enviadoConExito) {
             return (
                <div className="text-center animate-fade-in">
                    ...
                    <p className="mt-4 text-gray-700 dark:text-gray-300">El contrato de <span className="font-bold">{usuario?.nombreUsuario}</span> ha sido firmado y almacenado con validez digital.</p>
        ...
        return (
            <>
                <div className="text-center">
                    <h2 className="text-2xl font-black text-tkd-dark dark:text-white uppercase tracking-tight leading-none">Contrato Laboral</h2>
                ...
                <div className="space-y-4">
                    <div className="flex justify-between items-center px-1">
                        <label className="text-[10px] font-black uppercase text-gray-400 tracking-widest">Firma Autorizada Colaborador</label>
