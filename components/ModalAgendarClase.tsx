...
const ModalAgendarClase: React.FC<Props> = ({ abierto, onCerrar, onGuardar, bloqueActual, programas, sedes, usuarios }) => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        e.preventDefault();
        if (!formData.programaId || !formData.sedeId || !formData.instructorId) return;
        ...
        try {
            const bloqueFinal: BloqueHorario = {
                ...
                nombrePrograma: programas.find(p => p.id === formData.programaId)?.nombre
            };
    ...
    return (
        <div className="fixed inset-0 z-[120] flex items-center justify-center bg-tkd-dark/95 p-4 animate-fade-in backdrop-blur-sm">
            <div className="bg-white dark:bg-gray-900 rounded-[3rem] shadow-2xl w-full max-w-lg p-10 space-y-8 overflow-hidden relative border border-gray-100 dark:border-white/5">
