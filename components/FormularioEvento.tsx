...
const VALORES_POR_DEFECTO: FormData = {
  nombre: '',
  descripcion: '',
  lugar: '',
...
const FormularioEvento: React.FC<Props> = ({ abierto, onCerrar, onGuardar, eventoActual, cargando }) => {
  ...
  const { register, handleSubmit, formState: { errors, isValid }, reset, setValue, watch, formState } = useForm<FormData>({
  ...
  // Solo resetear cuando el modal se abre por primera vez o cambia el eventoActual
  useEffect(() => {
    const currentId = eventoActual?.id || 'nuevo';

    if (abierto && initializationRef.current !== currentId) {
      console.log("[FormularioEvento] Inicializando formulario para:", currentId);

      // Si hay un borrador, no sobreescribimos con reset, dejamos que useAutosave lo maneje
      // O si no hay borrador, inicializamos con los valores del evento o por defecto
      if (!hasDraft) {
        reset(eventoActual ? {
          nombre: eventoActual.nombre,
          lugar: eventoActual.lugar,
  ...
  }, [abierto, eventoActual, hasDraft, reset]);
  ...
  useEffect(() => {
    if (abierto) {
      ...
      initializationRef.current = null; // Limpiar el ref al cerrar
      setVisible(false);
    }
  }, [abierto]);
