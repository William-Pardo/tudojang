...
import { IconoCerrar, IconoCandado, IconoUsuario, IconoGuardar, IconoEmail, IconoInformacion, IconoCasa, IconoOjoAbierto, IconoOjoCerrado, IconoWhatsApp, IconoContrato } from './Iconos';
...
const crearEsquemaValidacion = (esEdicion: boolean) => {
  return yup.object({
    ...
    // Nuevos campos de contrato integrados
    valorPago: yup.number().when('rol', {
      is: (val: string) => val !== RolUsuario.SuperAdmin,
      then: (s) => s.min(1000, 'Monto inválido.').required('El valor de pago es obligatorio.'),
      otherwise: (s) => s.optional(),
    }),
    tipoVinculacion: yup.string().oneOf(Object.values(TipoVinculacionColaborador)).required('Defina el tipo de vinculación.'),
    fechaInicio: yup.string().required('Defina la fecha de inicio de labores.'),
  }).required();
};
...
const FormularioUsuario: React.FC<Props> = ({ abierto, onCerrar, onGuardar, usuarioActual, cargando }) => {
  ...
  const { register, handleSubmit, formState: { errors, isValid }, reset, watch, setValue } = useForm<any>({
    ...
    if (abierto) {
      if (usuarioActual) {
        reset({
          ...
          tipoVinculacion: usuarioActual.contrato?.tipoVinculacion || TipoVinculacionColaborador.Mes,
  ...
  const onSubmit = async (data: any) => {
    // REGLA DE SEGURIDAD: No permitir que el admin se cree a sí mismo de nuevo
    if (!esEdicion && data.email.toLowerCase().trim() === adminActual?.email.toLowerCase().trim()) {
      alert("Error de Seguridad: Usted ya es el administrador principal. No puede crear otro perfil con su mismo correo.");
      return;
    }

    // Estructurar el contrato dentro de los datos
    const payload = {
      ...data,
      contrato: {
        ...
        firmado: false,
        lugarEjecucion: tenant?.ciudad || ''
      },
      estadoContrato: 'Pendiente'
    };

    onGuardar(payload, usuarioActual?.id);

    // Si es nuevo, enviar correo de bienvenida con credenciales
    if (!esEdicion) {
      try {
        await enviarEmailBienvenida({
          email: data.email,
          nombreClub: tenant?.nombreClub || 'Tu Academia',
          passwordTemporal: data.contrasena,
          slug: tenant?.slug || ''
        });
        console.log("Correo de bienvenida enviado al nuevo colaborador.");
      } catch (err) {
        console.error("Fallo al enviar notificación al colaborador:", err);
      }
    }
  };
    ...
    <div className={`fixed inset-0 z-50 flex items-center justify-center bg-black transition-opacity duration-200 ease-out ${visible ? 'bg-opacity-70' : 'bg-opacity-0'}`} aria-modal="true" role="dialog" onClick={handleClose}>
      <div className={`bg-white dark:bg-gray-900 rounded-3xl shadow-2xl w-full max-w-md mx-4 max-h-[95vh] flex flex-col transform transition-all duration-200 ease-out ${visible ? 'opacity-100 scale-100' : 'opacity-0 scale-95'}`} onClick={e => e.stopPropagation()}>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="p-6 overflow-y-auto space-y-5">
          ...
          {!esEdicion && (
            <div className="p-5 bg-tkd-blue/5 rounded-2xl border border-tkd-blue/10 space-y-4 animate-fade-in">
              <div className="flex items-center gap-3">
                <IconoContrato className="w-5 h-5 text-tkd-blue" />
                <h3 className="text-xs font-black uppercase text-tkd-blue tracking-tighter">Parámetros de Contratación</h3>
              </div>

              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="text-[9px] font-black uppercase text-gray-400 mb-2 block">Monto Pago (COP)</label>
                  <input type="number" {...register('valorPago')} className={inputClasses.replace('pl-10', 'pl-4')} />
                  <FormInputError mensaje={errors.valorPago?.message} />
                </div>
                <div>
                  <label className="text-[9px] font-black uppercase text-gray-400 mb-2 block">Modalidad</label>
                  <select {...register('tipoVinculacion')} className="w-full bg-white dark:bg-gray-800 border-gray-300 dark:border-gray-600 rounded-xl p-3 text-xs font-black uppercase outline-none focus:ring-2 focus:ring-tkd-blue">
                    <option value={TipoVinculacionColaborador.Mes}>Mes</option>
                    <option value={TipoVinculacionColaborador.Hora}>Hora</option>
                    <option value={TipoVinculacionColaborador.Evento}>Evento</option>
                  </select>
                </div>
              </div>

              <div>
                <label className="text-[9px] font-black uppercase text-gray-400 mb-2 block">Fecha Inicio de Labores</label>
                <input type="date" {...register('fechaInicio')} className={inputClasses.replace('pl-10', 'pl-4')} />
              ...
              <p className="text-[9px] font-bold text-gray-400 uppercase leading-tight italic">
                * Al guardar, se enviará un correo automático al colaborador con sus accesos y se le recordará su responsabilidad técnica.
              </p>
            </div>
          )}
        </form>
